...
function solutionImplmentation(name, currentIndex ,possibleIndices) {
  let counter = 0;
  let leftCounter, rightCounter;
  if( possibleIndices.length > 0 ) { // recursive case
    counter += matchCharacter(name, currentIndex);
    currentIndex = calcNextIndex(name, currentIndex, possibleIndices, "LEFT");
    leftCounter = solutionImplmentation(name, currentIndex,  )
  }

}

function updatePossibleIndices(name, currentIndex, possibleIndices) {
  
}

function solution(name){
  let possibleIndices = Array.from(name);
  possibleIndices = createPossibleIndices(name, possibleIndices);
  solutionImplmentation(name, 0, possibleIndices);
}
...
function createPossibleIndices(name, possibleIndices) {
  let tempIndices = [];
  possibleIndices.forEach((targetIndex) => {
    if (name[targetIndex] !== "A" && targetIndex !== 0)
